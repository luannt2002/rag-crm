...
| **P3** | **Bad formatting / styling anomalies** | Owner uses bold-14 as heading (no `#`); rule-based block-detect mis-fires | **Tenant Profiling**: per-bot meta-rules in DB (`heading_indicator`, `table_separator`) read by `bot_id`, not hardcoded global rules. + heuristic heading-mapping (upper+short→`##`) | parser adapters; `chunking/blocks.py`; no per-bot styling config | 🔴 OPEN — no tenant-profiling layer; rules are global |
